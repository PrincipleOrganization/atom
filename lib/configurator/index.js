...
var getUsedConfiguration = function(callback) {
  var db = store.getDB();
  db.get(
    'SELECT * FROM configs WHERE use=?',
    1,
    function(err, row) {
      var error = null;
      if (err) {
...
var addNewConfig = function(cnf, callback) {
  ...
  db.serialize(function() {
    if (cnf.use == 1) {
      db.run('UPDATE configs SET use=0');
    }
...
var updateConfig = function(cnf, callback) {
  var db = store.getDB();
  db.serialize(function() {
    if (cnf.use == 1) {
      db.run('UPDATE configs SET use=0');
    ...
    var _config = db.prepare('REPLACE INTO configs (id, name, port_dev, port_prod, db_vendor, db_host, db_port, db_base_dev, db_base_prod, db_maxcount, socket_use, socket_port, intervalCommands_use, intervalCommands_interval, use, canBeDeleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    _config.run(
      cnf.id,
      cnf.name,
      '4000',
...
var updateDevice = function(device, callback) {
  var db = store.getDB();
  db.serialize(function() {
    var _devices = db.prepare('REPLACE INTO devices (id, name, path, tableName, vendor, model, units, baudRate, dataBits, stopBits, parity, rtscts, xon, xoff, xany, flowControl, bufferSize, use) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    _devices.run(
      device.id,
      device.name,
      device.path,
...
var addNewPlugin = function(plugin, callback) {
  var db = store.getDB();
  db.serialize(function() {
    var _plugins = db.prepare('INSERT OR REPLACE INTO plugins (tableName, vendor, model, executeFunction, params) VALUES (?, ?, ?, ?, ?)');
    _plugins.run(
      plugin.tableName,
      plugin.vendor,
      plugin.model,
...
var updatePlugin = function(plugin, callback) {
  var db = store.getDB();
  db.serialize(function() {
    var _plugins = db.prepare('REPLACE INTO plugins (id, tableName, vendor, model, executeFunction, params) VALUES (?, ?, ?, ?, ?, ?)');
    _plugins.run(
      plugin.id,
      plugin.tableName,
      plugin.vendor,
...
exports.getUsedConfiguration = getUsedConfiguration;
